...
<script>
    ...
    function displayData(response) {
        ...
        var stillInQueue = 0;
        var sn = 1;
        content = "<table><thead><tr><th>S/N</th><th>Names</th><th>Age (Years)</th><th>PREVIOUS LOCATION</th><th>ASSIGNED PROVIDER</th><th>WAITING TIME</th><th>STATUS</th><th colspan='2'>ACTION</th></tr></thead><tbody>";
        for (i = 0; i < response.patientQueueList.length; i++) {
            ...
            var urlToForm = '${ui.pageLink("htmlformentryui","htmlform/editHtmlFormWithStandardUi",[patientId: "patientIdElement", encounterId: "encouterIdElement"])}'.replace("patientIdElement", patientQueueListElement.patientId).replace("encouterIdElement", patientQueueListElement.encounterId);
            ...
            var waitingTime = getWaitingTime(patientQueueListElement.dateCreated);
           	console.log(patientQueueListElement);
            content += "<tr>";
            content += "<td>" + sn + "</td>";
            ...
            content += "<td>" + patientQueueListElement.status + "</td>";
            content += "<td>";
            content += "<i style=\"font-size: 25px;\" class=\"icon-dashboard view-action\" title=\"Goto Patient's Dashboard\" onclick=\"location.href = 'urlToPatientDashboard'\"></i>".replace("urlToPatientDashboard", urlToPatientDashBoard);
            content += "</td>";           
            if((patientQueueListElement.isProviderLab) || (patientQueueListElement.isProviderWithPharmacyRole) || (patientQueueListElement.isProviderClinician)){	           
	            content += "<td>";
	            content += "<i style=\"font-size: 25px;\" class=\"icon-edit edit-action\" title=\"Edit Form\" onclick=\"location.href = 'urlToForm'\"></i>".replace("urlToForm", urlToForm);
    			content += "</td>";
    		}	
            content += "</tr>";
            stillInQueue+=1;
